...
class ProducedQtyEmployee(models.Model):
    ...
    attendance_ids = fields.One2many('produced_qty.attendance', 'employee_id', help='list of attendances for the employee')
    last_attendance_id = fields.Many2one('produced_qty.attendance', compute='_compute_last_attendance_id')
    ...
    work_center_id = fields.Many2many(string=_("Work Center"), comodel_name='mrp.workcenter')
    ...
    def attendance_scan(self, barcode):
        ...
        return employee and employee.attendance_action('production_quantity_tracking_by_employee.hr_attendance_action_kiosk_mode') or \
            {'warning': _('No employee corresponding to barcode %(barcode)s') % {'barcode': barcode}}

    @api.multi
    def qty_attendance_manual(self, next_action, entered_pin=None):
        self.ensure_one()
        if not (entered_pin is None) or self.env['res.users'].browse(SUPERUSER_ID).has_group('production_quantity_tracking_by_employee.group_produced_qty_attendance_use_pin') and (self.user_id and self.user_id.id != self._uid or not self.user_id):
    ...
    def attendance_action(self, next_action):
        ...
        """
        self.ensure_one()
        action_message = self.env.ref('production_quantity_tracking_by_employee.qty_attendance_action_greeting_message').read()[0]
        action_message['previous_attendance_change_date'] = self.last_attendance_id and (self.last_attendance_id.check_in) or False
        ...
        workorder_obj.get_active_work_center()
    ...
    def attendance_action_change(self):
        ...
        return self.env['produced_qty.attendance'].create(vals)
    ...
    def _init_column(self, column_name):
        ...
        if column_name not in ["barcode", "pin"]:
            super(ProducedQtyEmployee, self)._init_column(column_name)
        else:
